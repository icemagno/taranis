...
public class PostgreSqlDBManagerFactory implements DBManagerFactory {
	
	private PntcConfig config;
	
	public PostgreSqlDBManagerFactory(PntcConfig config) {
		this.config = config;
	}

	@Override
	public DBManager createDBManager() throws Exception {
		return new PostgreSqDBManager(config);
	}
